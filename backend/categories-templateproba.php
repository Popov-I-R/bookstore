...
$category_id = $_GET['id'];
$query = "SELECT books.id, books.title, books.image, books.price, categories.title FROM books INNER JOIN book_category ON books.id=book_category.book_id INNER JOIN categories ON book_category.category_id=categories.id WHERE book_category.category_id =$category_id ";
...
/* categories title при мен може и да може да се махне, при Иво е трябвало за заглавието, но това е неговата заявка */
