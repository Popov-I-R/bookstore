...
$stmp = $conn->prepare('UPDATE books SET isbn=?,title=?,year=?,description=?,image=?,publisher=? WHERE id=? ');
$stmp->bind_param('sssssdisi',$isbn, $title, $year, $description, $image, $price, $publisher, $date_created,  $id );
$result = $stmp->execute();    
}
...
//}
    ...
    $stmt = $conn->prepare("UPDATE book_category(book_id, category_id) WHERE book_id=?");
    $conn->begin_transaction();        
